...
namespace GameProject
{
    ...
    public class Game1 : Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < bears.Count; i++)
            {
                for (int j = i + 1; j < bears.Count; j++)
                {
                    ...
                    {
                        var collisionInfo = CollisionUtils.CheckCollision(gameTime.ElapsedGameTime.Milliseconds,
        ...
        public static Texture2D GetProjectileSprite(ProjectileType type)
        {
            ...
            return type == ProjectileType.TeddyBear ? teddyBearProjectileSprite : frenchFriesSprite;
        }
        ...
        private void SpawnBear()
        {
            ...
            var speed = GameConstants.MinBearSpeed + RandomNumberGenerator.NextFloat(GameConstants.BearSpeedRange);

            var angleInRadians = RandomNumberGenerator.NextFloat(2 * (float)Math.PI);

            var velocity = new Vector2(speed * (float)Math.Cos(angleInRadians),
                speed * (float)Math.Sin(angleInRadians));

            // create new bear
            var newBear = new TeddyBear(Content, @"graphics/teddybear", x, y, velocity, null, null);
            ...
            {
                x = GetRandomLocation(GameConstants.SpawnBorderSize,
                (graphics.PreferredBackBufferWidth - 2 * GameConstants.SpawnBorderSize));
                y = GetRandomLocation(GameConstants.SpawnBorderSize,
                    (graphics.PreferredBackBufferHeight - 2 * GameConstants.SpawnBorderSize));
                newBear.X = x;
                newBear.Y = y;
            }
